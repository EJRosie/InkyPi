...
OPEN_METEO_FORECAST_URL = "https://api.open-meteo.com/v1/forecast?latitude={lat}&longitude={long}&daily=temperature_2m_max,temperature_2m_min&timezone=auto&forecast_days=1&temperature_unit=fahrenheit"

class CalendarWithWeather(BasePlugin):
    ...
    def generate_image(self, settings, device_config):
        ...
        # Fetch weather data for today's high/low
        today_high = None
        today_low = None
        ...
        if lat and long:
            try:
                weather = self.fetch_today_weather(float(lat), float(long))
                today_high = weather.get('high')
                today_low = weather.get('low')
        ...
        image = self.render_image(dimensions, "calendar_with_weather.html", "calendar_with_weather.css", template_params)
    ...
    def fetch_today_weather(self, lat, long):
        """Fetch today's high and low temperature in °F from Open-Meteo API."""
        ...
        high = int(round(temp_max[0])) if temp_max else None
        low = int(round(temp_min[0])) if temp_min else None
